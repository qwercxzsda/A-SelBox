...
def download_report_document(
    ...
    """
    Downloads the report document using the provided client and saves it to the specified output path.
    ...
    logger.info(f"Downloaded settlement report to: {output_path}")
...
if __name__ == "__main__":
    ...
    for page in report_pages:
        for report in page.payload.get("reports", []):
            ...
            report_id_document_id.append(
                (report["reportId"], report["reportDocumentId"])
            )
            j += 1
